...
export class DisposableStore implements IDisposable {
  ...
  public add<T extends IDisposable>(t: T): T {
    ...
    if (this._isDisposed) {
      console.warn(
        new Error('Registering disposable on object that has already been disposed of').stack
      )
      t.dispose()
    } else {
...
interface IDisposer extends IDisposable {
  (): any
}

export function toDisposer<T extends IDisposable | Function>(disposable: T): IDisposer {
  let disposer: any = disposable
  if (typeof disposer === 'function') {
    if (typeof (disposer as any).dispose === 'function') {
      return disposer as any
    }
    disposer.dispose = () => {
      disposer()
      disposer.dispose = Disposable.None.dispose
    }
    return disposer
  }

  disposer = () => dispose(disposable as any)
  disposer.dispose = () => {
    disposer()
    disposer.dispose = Disposable.None.dispose
  }
