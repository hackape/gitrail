...
export function useEffect(fn: Function, deps?: any[]) {
  ...
  if (deps !== undefined && !Array.isArray(deps)) {
    throw new Error(
      '`deps` argument in `useEffect(callback, deps)` must be array'
    )
  }

  const slot =
    bucket.effects[bucket.effectIdx] || (bucket.effects[bucket.effectIdx] = [])
...
export function useMemo<T>(factory: () => T, deps: any[] | undefined): T {
  ...
  const slot =
    bucket.memos[bucket.memoIdx] || (bucket.memos[bucket.memoIdx] = [])
...
export function useRef<T>(initValue: T = null) {
